...
function onTick()
	if GameData.startedmenu == nil then
		GameData.startedmenu = GameData.startedmenu or 1
	end
	player:setFrame(50)
...
function onEvent(eventName)
	...
	if eventName == "ThemeAlreadyInUse" then
		littleDialogue.create({text = "<boxStyle smbx13><setPos 400 32 0.5 -1.7>This theme is in use.<question OkayToMenuTheme>", pauses = false, updatesInPause = true})
	end
...
function onDraw()
	Graphics.drawBox{x=748, y=5, width=47, height=20, color=Color.black..0.5, priority=5}
