...
--require 'ext/wifi/socket'
require 'ext/wifi/ztact'
...
local get, set, tohex = ztact.get, ztact.set, ztact.tohex
...
function resolver:rr ()    -- - - - - - - - - - - - - - - - - - - - - - - -  rr
  ...
  -- print ('ttl', rr.ttl, rr.name)  os.exit (1)
...
function resolver:getsocket (servernum)    -- - - - - - - - - - - - - getsocket
  ...
  sock:setsockname ('*', 0)
  -- print (sock:getsockname ())  os.exit (1)
...
function dns.rr (t)    -- - - - - - - - - - - - - - - - - - - - - - - - - -  rr
  return setmetatable (t or {}, rr_metatable)  end
